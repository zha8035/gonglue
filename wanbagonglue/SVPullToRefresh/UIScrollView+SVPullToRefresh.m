...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGFloat leftViewWidth = MAX(self.arrow.bounds.size.width,self.arrow.bounds.size.width);
        ...
        self.arrow.frame = CGRectMake(100,
...
- (UIImageView *)arrow {
    if(!_arrow) {
		...
		_arrow = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 32*image.size.width/image.size.height, 32)];
        ...
        [_arrow setAnimationImages:images];
        [_arrow setAnimationDuration:0.8];
    }
    return _arrow;
}
